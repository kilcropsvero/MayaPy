...
def L_arm(*args):
    ...
    # (END OF THE RIGHT ARM)
        ...
        # Create the Iksolver:
        ikarm = mc.ls(ik[0], ik[2])
        mc.select(ikarm)
        IKRP = mc.ikHandle(name="L_arm_IkRp", solver="ikRPsolver")
        ...
        # ZeroOut ikcc
        ikcc_off = mc.group(ikcc, n="grpL_arm_CCZERO", empty=True)
        ...
        mc.xform("grpL_arm_PVZERO", cpc=1)
        ...
        grp=mc.group(clav, n="L_clavicle_FK_CCZERO")
        mc.setAttr("{}.overrideEnabled".format(grp), 1)
        mc.setAttr("{}.overrideColor".format(grp), 6)
        mc.xform(grp, worldSpace=True, rotation=(temp_rot[0], temp_rot[1], temp_rot[2]))
        ...
        mc.select("temp0", hi=True) #temp was a generic name to list and then rename.
        ...
        if mc.radioButton("rarm", query=True, select=True):
            ...
            def R_arm():
                ...
                mc.xform(ikpv_off, worldSpace=True, translation=(temp_pos2[0], temp_pos2[1], temp_pos2[2]))
                mc.setAttr("{}.translate".format(ikpv), 0, 0, 0)
                mc.xform("grpR_arm_PVZERO", cpc=1)
                ...
                mc.delete(temp_constraint)
                temp_rot = mc.xform(clav, rotation=True, query=True, worldSpace=True)
                grp = mc.group(clav, n="R_clavicle_FK_CCZERO")
                mc.setAttr("{}.overrideEnabled".format(grp), 1)
                mc.setAttr("{}.overrideColor".format(grp), 13)
                mc.xform(grp, worldSpace=True, rotation=(temp_rot[0], temp_rot[1], temp_rot[2]))
                mc.setAttr("{}.rotate".format(clav[0]), 0, 0, 0)
                ...
                # query master twist joints position
                mc.select(cl=True)
                pPos = mc.joint("R_upTwist_01_JC", query=True, position=True)
                cPos = mc.joint("R_upTwist_02_JC", query=True, position=True)
                ...
                mc.parent(world=True)
                mc.delete("temp0")
                mc.rename("temp1", "R_uparmTwist_C_JJ")
                mc.rename("temp2", "R_uparmTwist_B_JJ")
                mc.rename("temp3", "R_uparmTwist_A_JJ")
                upTwi = mc.ls(sl=True)
                mc.parent(upTwi, rjj[1])

                # LOWERARM: (repeating the process)
                # query master twist joints position
                mc.select(cl=True)
                pPos = mc.joint("R_loTwist_01_JC", query=True, position=True)
                cPos = mc.joint("R_loTwist_02_JC", query=True, position=True)
                # Vector
                numberOfJoints = 4
                jVector = [0, 1, 2]
                jVector[0] = (cPos[0] - pPos[0]) / numberOfJoints
                ...
                mc.parent(world=True)
                mc.delete("temp0")
                mc.rename("temp1", "R_loarmTwist_A_JJ")
                mc.rename("temp2", "R_loarmTwist_B_JJ")
                mc.rename("temp3", "R_loarmTwist_C_JJ")
                loTwi = mc.ls(sl=True)
                mc.parent(loTwi, rjj[2])
